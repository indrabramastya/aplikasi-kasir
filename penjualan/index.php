...
// jika ada barcode dikirim
$kode   = @$_GET['barcode'] ? @$_GET['barcode'] : '';
if ($kode) {
    $tgl    = $_GET['tgl'];
    $dataBrg = mysqli_query($koneksi, "SELECT * FROM file_barang WHERE barcode= '$kode'");
    ...
    if (simpan($_POST)) {
        echo "<script>
            ...
            window.onload = function(){
                let win = window.open('../report/formulir_penjualanretail.php?nota=$nota', 'Struk Belanja', 'width=260,height=400,left=10,top=10','_blank');
                if(win){
                    win.focus();
                        ...
                        <div class="card card-outline card-warning p-3">
                            ...
                            <div class="form-group row mb-2">
                                ...
                                <div class="col-sm-10 input-group">
                                    <input type="text" name="barcode" id="barcode" value="<?= @$_GET['barcode'] ? $_GET['barcode'] : '' ?>" class="form-control" placeholder="masukkan barcode">
                ...
                <div class="card card-outline card-success table-responsive px-2">
                    <table class="table table-sm table-hover text-nowrap">
                        ...
                        <tbody>
                            <?php
                            $no         = 1;
                            $brgDetail  = getData("SELECT * FROM detail_penjualan WHERE no_jual = '$nojual'");
                            foreach ($brgDetail as $detail) { ?>
                                <tr>
                                    ...
                                    <td class="text-center">
                                        <a href="?barcode=<?= $detail['barcode'] ?>&idjual=<?= $detail['no_jual'] ?>&qty=<?= $detail['qty'] ?>&tgl=<?= $detail['tgl_jual'] ?>&msg=deleted" class="btn btn-sm btn-danger" title="hapus barang" onclick="return confirm ('Apakah anda yakin akan menghapus barang ini?')"><i class="fa fa-trash"></i></a>
                                    </td>
                                </tr>
                            <?php
                            }
