...
if ($kode) {
    $selectBrg = getData("SELECT * FROM file_barang WHERE id_barang = '$kode'")[0];
}
            ...
            <div class="row mb-2">
                ...
                <div class="col-sm-6">
                    <ol class="breadcrumb float-sm-right">
                        ...
                        <li class="breadcrumb-item"> Tambah Pembelian</a></li>
                    </ol>
                </div><!-- /.col -->
            </div><!-- /.row -->
        </div><!-- /.container-fluid -->
                ...
                <div class="row">
                    ...
                    <div class="col-lg-6">
                        <div class="card card-outline card-danger pt-3 px-3 pb-2">
                            ...
                            <h1 class="font-weight-bold text-right" style="font-size:40pt;"> <input type="hidden" name="total" value="<?= totalBeli($noBeli) ?>"> <?= number_format(totalBeli($noBeli), 0, ',', '.') ?></h1>
                        </div>
                    </div>
                </div>
                    ...
                    <div class="row">
                        ...
                        <div class=" col-lg-1">
                            <div class="form-group">
                                ...
                                <input type="number" name="stok" class="form-control form-control-sm" id="stok" value="<?= @$_GET['pilihbrg'] ? $selectBrg['stok'] : '' ?>" readonly>
                            </div>
                        </div>
                        <div class=" col-lg-1">
                            <div class="form-group">
                ...
                <div class="card card-outline card-success table-responsive px-2">
                    <table class="table table-sm table-hover text-nowrap">
                        ...
                        <tbody>
                            <?php
                            $no         = 1;
                            $brgDetail  = getData("SELECT * FROM detail_pembelian WHERE no_beli = '$noBeli'");
                            foreach ($brgDetail as $detail) { ?>
                                <tr>
                                    ...
                                    <td class="text-center">
                                        <a href="?idbarang=<?= $detail['kode_brg'] ?>&idbeli=<?= $detail['no_beli'] ?>&qty=<?= $detail['qty'] ?>&tgl=<?= $detail['tgl_beli'] ?>&msg=deleted" class="btn btn-sm btn-danger" title="hapus barang" onclick="return confirm ('Apakah anda yakin akan menghapus barang ini?')"><i class="fa fa-trash"></i></a>
                                    </td>
                                </tr>
                            <?php
                            }
                        ...
                        <div class="form-group row mb-2">
                            ...
                            <div class="col-sm-9">
                                <select name="suplier" id="suplier" class="form-control form-control-sm">
                                    ...
                                    <?php
                                    $supliers = getData("SELECT * FROM file_supplier");
                                    foreach ($supliers as $suplier) { ?>
                                        <option value="<?= $suplier['nama'] ?>"><?= $suplier['nama'] ?></option>
                                    <?php
                                    }
                                    ?>
                                </select>
